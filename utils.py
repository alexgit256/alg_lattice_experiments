...
_sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_320 = Integer(320); _sage_const_1000 = Integer(1000); _sage_const_0 = Integer(0); _sage_const_2p = RealNumber('2.'); _sage_const_1p = RealNumber('1.'); _sage_const_4 = Integer(4); _sage_const_520 = Integer(520); _sage_const_12 = Integer(12); _sage_const_0p995 = RealNumber('0.995'); _sage_const_14 = Integer(14); _sage_const_0p5 = RealNumber('0.5'); _sage_const_350 = Integer(350); _sage_const_33 = Integer(33); _sage_const_1p03 = RealNumber('1.03'); _sage_const_300 = Integer(300)
...
try:
    enorm_vector_over_numfield(vector(CyclotomicField(_sage_const_2 ),[_sage_const_1 ]))
except NameError:
...
def minkowski_embedding(a, truncated=True):
    ...
    if truncated:
        return [s(a) for s in sigmas[:len(sigmas)/_sage_const_2 ]]
    else:
...
def short_lattice_vectors(B, nr_solutions=_sage_const_1 ,  task_id=None, verbose=verbose, approx=None, radius=_sage_const_0p995  ):
    ...
    if approx is None:
        ...
        assert approx in ZZ and approx > _sage_const_0 , f"approx sould be non-negative integer!"
        block_sizes = [i for i in range(_sage_const_4 ,int(min(n,m,approx)),_sage_const_2 )] + [ min(n,m,approx) ]
    ...
    if verbose:
        GSO_M.update_gso()
        gh = gaussian_heuristic([GSO_M.get_r(i,i) for i in range(n)])
        print('gh:', gh**_sage_const_0p5  / l, 'true len:', GSO_M.get_r(_sage_const_0 ,_sage_const_0 )**_sage_const_0p5  / l )
    ...
    R = GSO_M.get_r(_sage_const_0 , _sage_const_0 )*radius
...
class PseudoBasis:
    ...
    def bitlength(self):
        ...
        return ( log(tmp,_sage_const_2 ) + log(Tmp.denominator(),_sage_const_2 ) ).n()
    ...
    def det(self):
        ...
        return norm( det( self.A * self.A.conjugate_transpose() ) )**(_sage_const_1 /_sage_const_2 ) * norm( self.steinitz_class_member() )
    ...
    def make_almost_free( self ):
        ...
        Is = self.Is
        K = self.Field
        for i in range( A.nrows()-_sage_const_1  ):    # for every submodule A*x+B*y -> OK*x' + AB*y'
            aid, bid = Is[i], Is[i+_sage_const_1 ]

            sa, sb = ideal_denominator(aid), ideal_denominator(bid)   #scale coefficients

            aid, bid = sa*Is[i], sb*Is[i+_sage_const_1 ]     #scale the ideals
            A[i], A[i+_sage_const_1 ] = A[i]/sa, A[i+_sage_const_1 ]/sb   #dont' forget to scale the vectors

            #corollary 1.2.11 from Cohen
            aid_m1 = aid**-_sage_const_1
            scale = ideal_denominator(aid_m1)
            aid_m1 *= scale
            if log(scale,_sage_const_2 ).n() > _sage_const_350 :    #if "PariError: the PARI stack overflows" is about to happen...
                pari.default("parisizemax", _sage_const_2 **_sage_const_33 )
                pari.allocatemem(_sage_const_2 **_sage_const_33 )
            ...
            e, f = pari.idealaddtoone(K,a*aid**-_sage_const_1 ,bid)
            e, f = K(e), K(f)
            assert f in bid
            assert e+f == K(_sage_const_1 )
            b, c = f, -_sage_const_1
            d = e/a

            A[i], A[i+_sage_const_1 ] = A[i]*a + A[i+_sage_const_1 ]*b, A[i]*c + A[i+_sage_const_1 ]*d
            Is[i], Is[i+_sage_const_1 ] = Ideal(K,K(_sage_const_1 )), Is[i]*Is[i+_sage_const_1 ]
        self.A = A
        self.Is = Is
    ...
    def gaussian_heuristic(self):
        ...
        return (gh )**(_sage_const_1 /_sage_const_2 ) / l

    def minkowski_bound(self):
        dim = self.rank * self.Field.degree()
        return sqrt(dim) * (self.det())**(_sage_const_1 /dim)

    def short_elements(self, nr_solutions=_sage_const_1  , algebraic_form=True,task_id=None, coords=True, verbose=verbose, approx=None, radius=_sage_const_1p03  ):
