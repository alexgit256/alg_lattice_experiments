...
def nearest_P_smooth_number(N,P=None):
    p_max = None
    if P is None:
        p_max = max( 3, round( log(log(N,2),2) ) )  #heuristically obtained constant
        P = gen_fact_base( p_max )
    if abs(ln(N)) < 10:
        scale = round( 10 + 2*log(N,2) )  #heuristically obtained constant
...
def gen_smooth_nearly_unit_ideal( K, n=12, B=10^10 ):
    scale=4*log(B,2)^2    #heuristically obtained constant
...
def arakelov_crawl(II, s, smooth_unit=False):
    ...
    if smooth_unit:
        ...
        assert abs( ln( abs(norm(U)) ) )<precision_trashold, f"Not a unit! {abs( ln( abs(norm(U)) ) ).n()}"
        return U*II
    else:
        ...
        assert( abs( ln(norm(u_stash))-ln(norm(u)) ) < precision_trashold ), f"u is not a unit! Diff: { abs(ln(norm(u_stash))-ln(norm(u))).n()}"

    assert abs(ln(u.norm())) < precision_trashold, f"arakelov_crawl - precision issues: {abs(ln(u.norm())).n()}"
...
def arakelov_rand_walk(II, B, s, N, normalize=True, smooth_unit=False):
    """
    Performs the Arakelov random walk with parameter B - norm of randomizing ideals, s - the dispersion of the crawl step, and N - amount of jump steps.
    If normalize - the norm of randomized ideal is not too far from the initial one (on log scale). If smooth_unit - arakelov_crawl multiplies ideal
    by the ideal of factor_base smooth norm.
    """
    norm_stash = II.norm()
    for i in range(N):
